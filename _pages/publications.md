...
{% assign highlighted_publications = publications | where: "is_highlighted", true %}

{% if highlighted_publications.size > 0 %}
  <div class="pub-highlight">
    <div class="pub-highlight-head">
      <h2>Highlighted publications</h2>
      <p class="pub-highlight-sub">A few recent pieces I am especially excited about.</p>
    </div>

    <div class="pub-highlight-grid">
      {% for post in highlighted_publications %}
        ...
        {% assign blurb = post.blurb | default: post.short_citation | default: post.citation %}

        <article class="pub-highlight-card">
          <div class="pub-highlight-chip">
            <span class="pub-conf">{{ conf }}</span>
            <span class="pub-year">{{ pub_year }}</span>
          </div>

          <h3 class="pub-highlight-title"><a href="{{ base_path }}{{ post.url }}">{{ post.title }}</a></h3>

          <p class="pub-meta">{{ blurb }}</p>
...
<h2 class="pub-section-title">All publications</h2>
<div class="pub-list">
  {% for post in publications %}
    {% unless post.is_highlighted %}
      {% assign conf = post.venue_short | default: post.abbreviation | default: post.venue %}
      {% assign pub_year = post.pub_year | default: post.date | date: "%Y" %}
